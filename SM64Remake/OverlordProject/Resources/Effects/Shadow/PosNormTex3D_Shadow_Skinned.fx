...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	for(int i = 0; i < 4; ++i)
	{
		...
		if(boneIndex >= 0)
		{
			transformedPosition += mul(float4(input.pos,1.0f), gBones[boneIndex] ) * input.BoneWeights[i];
			transformedNormal += mul(input.normal, (float3x3)gBones[boneIndex] ) * input.BoneWeights[i];
		}
	}
